...
const scoredSubmissionsUnsorted: Omit<Submission, "source" | "rank">[] = [
  ...
  },
  {
    ...
    categories: [
      ...
      "Best New Dev Tool",
      "Private & Gasless payments for agents",
    ],
    description:
    ...
    highlights:
      "Solves two critical problems simultaneously: gasless transactions and payment privacy via Railgun integration. The description is technically detailed and the custom category perfectly describes a gap nobody else is addressing. Live and going multi-chain.",
  },
  {
    project: "Vincent",
  ...
  },
  {
    ...
    highlights:
      "Well-described architecture with gRPC API, normalized ingestion, configurable prompts, and Rust compilation path. 'Lighter than OpenClaw' with native multi-agent orchestration is a clear differentiator. Production-readiness details show maturity.",
  },
  {
  ...
  },
  {
    ...
    highlights:
      "'Call your OpenClaw like a phone call' is an immediately compelling pitch. The skill-based integration is clean and the accessibility layer makes agents usable by non-technical people. High show factor because a live phone call to an AI agent on stage is inherently entertaining.",
  },
  {
  ...
  },
  {
    ...
    highlights:
      "Clean execution with a polished macOS native app, bundled runtime for zero dependencies, and 160 tests showing engineering rigor. The vision-based file renaming and natural language commands show genuine AI integration beyond a wrapper. Practical daily-use tool.",
  },
  {
    project: "Klaus (Bits)",
...
const highlights = [
  {
    ...
    team: "Drew Mailen",
    reason:
      "The clear standout. Addresses the #1 existential risk to the agent ecosystem (malicious skills) with deployed smart contracts and live attack simulations.",
  },
  {
    project: "clawpay (b402)",
    team: "Mayur Chougule",
    reason:
      '\u201cStripe for AI agents\u201d with privacy. If agents are going to transact autonomously, they need this.',
  },
  {
    project: "Starkbot AI",
    team: "Andrew Mazzola",
    reason:
      'The most complete product in the field. Two-click K8s deploy, the only team with a demo video, and a live product.',
  },
  {
    project: "Clawdtalk",
    team: "Sonam Gupta",
    reason:
      "Best demo moment potential. Calling an AI agent on the phone live on stage is guaranteed entertainment.",
  },
  {
    project: "Xao Judge",
    team: "Kelvin McDaniel",
    reason:
      'Most crowd-pleasing demo concept. \u201cDid this contract get breached? Let\u2019s find out!\u201d is inherently dramatic.',
  },
];
...
const scoredKeys = new Set(
  scoredSubmissions.flatMap((s) => [
    normalize(s.project),
    normalize(s.team),
    // Also split multi-word team names (e.g. "David Sneider / Lit Protocol")
...
export default function NomineesPage() {
  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }: { data: Vote[] | null }) => {
        if (data) {
          // Normalize item_id to string (bigint comes back as number)
          setVotes(data.map((v: Vote) => ({ ...v, item_id: String(v.item_id) })));
        }
      });
  }, []);

  // Fetch nominations, merge with scored
  useEffect(() => {
  ...
  // Get net vote count for a nominee (by rank as string)
  ...
  const sortedSubmissions = [...filteredSubmissions].sort((a, b) => {
    ...
    // Scored entries first, then unscored
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#0a0a0a]/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          >
            ...
            <Image
              src="/logos/nebius-build-logo.png"
              alt="Nebius.Build"
              width={200}
              height={67}
      ...
      <section className="pt-28 pb-12 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-block px-4 py-2 border border-[#c8ff00]/30 text-sm tracking-widest uppercase text-[#c8ff00] mb-6">
            Nebius.Build 2026
          </div>
          ...
          <p className="text-lg text-white/50 max-w-2xl mx-auto mb-4">
            All nominations, speaker applications, and demo submissions for
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="border border-white/10 p-6">
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center text-sm">
              <div>
                <div className="text-[#c8ff00] font-semibold mb-1">25%</div>
                <div className="text-white/50">Innovation</div>
              </div>
              <div>
                <div className="text-[#c8ff00] font-semibold mb-1">25%</div>
                <div className="text-white/50">Agentic Depth</div>
              </div>
              <div>
                <div className="text-[#c8ff00] font-semibold mb-1">20%</div>
                <div className="text-white/50">Technical</div>
              </div>
              <div>
                <div className="text-[#c8ff00] font-semibold mb-1">20%</div>
              ...
              <div>
                <div className="text-[#c8ff00] font-semibold mb-1">10%</div>
                <div className="text-white/50">Vibes</div>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="flex flex-col items-center gap-0.5 px-3 py-3 flex-shrink-0">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          handleVote(String(s.rank), 1);
                        }}
                        className={`p-0.5 transition-colors ${
                          myVote(String(s.rank)) === 1
                            ? "text-[#c8ff00]"
                    ...
                    <button
                      onClick={() =>
                        setExpandedId(isExpanded ? null : s.rank)
                      }
                    ...
                    >
                        ...
                        <div
                          className={`w-10 h-10 flex items-center justify-center text-sm font-bold flex-shrink-0 ${
                            s.rank === 1
                              ...
                              : s.rank <= 3
                              ? "border border-[#c8ff00]/50 text-[#c8ff00]"
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          {s.award ? (
                            <span className="px-2 py-0.5 text-xs border border-[#c8ff00]/30 text-[#c8ff00]">
                              {s.award}
                            </span>
                          ) : (
                  ...
                  {isExpanded && (
                    <div className="px-5 pb-5 border-t border-white/5">
                      <div className="mt-5 space-y-5">
                        {/* Scores circles (only for scored entries) */}
                        {s.scores && (
                          <div>
                          ...
                          {s.award && (
                            <>
                              ...
                              <p className="text-sm text-white/70 mb-4 italic">
                                &ldquo;{s.awardDescription}&rdquo;
                              </p>
                            </>
                          )}
                          ...
                          {s.categories.length > 0 && (
                            <div className="flex flex-wrap gap-2 mb-4">
                              {s.categories.map((c) => (
                                <span
                                  key={c}
                                  className="px-2 py-0.5 text-xs border border-white/20 text-white/50"
                                >
                                  {c}
                                </span>
                              ))}
                          ...
                          <div className="flex gap-3">
                            {s.url && (
                              <a
                                ...
                                target="_blank"
                                rel="noopener noreferrer"
                                className="text-sm text-[#c8ff00] hover:text-[#c8ff00] transition-colors inline-flex items-center gap-1"
                              >
                                Project
      ...
      <footer className="border-t border-white/10 py-8 px-6">
        <div className="max-w-4xl mx-auto flex items-center justify-between text-sm text-white/30">
          ...
          <span>AI-judged, community-curated</span>
        </div>
      </footer>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm px-6">
          <div className="bg-[#0a0a0a] border border-white/20 p-8 max-w-md w-full relative">
            ...
            <p className="text-white/50 text-sm mb-6">
              Enter your name and email so we can track your votes.
            </p>
